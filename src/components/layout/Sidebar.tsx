...
export function Sidebar() {
  ...
  return (
    <>
      {/* Desktop sidebar */}
      <aside className="hidden md:flex w-16 bg-gray-900 flex-col items-center py-4 gap-2">
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-gray-900 border-t border-white/[0.06] safe-area-bottom">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-[#1c1c24] border border-white/[0.08] rounded-2xl shadow-2xl w-full max-w-md mx-4 overflow-hidden">
            <div className="flex items-center gap-3 px-6 py-5 border-b border-white/[0.06]">
              ...
              <div>
                <h2 className="text-[15px] font-semibold text-white">Clear All Data?</h2>
                <p className="text-[13px] text-white/50">This action cannot be undone</p>
              </div>
              <button
                onClick={() => setShowConfirmModal(false)}
                className="ml-auto w-8 h-8 rounded-lg flex items-center justify-center text-white/40 hover:text-white/70 hover:bg-white/[0.06] transition-colors"
              >
                <X size={18} />
              </button>
            </div>
